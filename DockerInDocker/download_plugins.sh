#!/bin/bash
echo "=================================================================="
echo "                  MediaWiki Plugin Downloader"
echo "                             by              "
echo "
     _____)                               __     __)
   /                                     (, /|  /|   ,
...
if [ -d /opt/bitnami/Mediawiki/extensions ]
then
    cd /opt/bitnami/Mediawiki/extensions
elif [ -d /opt/bitnami/Mediawiki/Extensions ]
then
    cd /opt/bitnami/Mediawiki/Extensions
else
    mkdir -p /opt/bitnami/Mediawiki/extensions 
    cd /opt/bitnami/Mediawiki/extensions
fi
...
echo 'Downloading MediaWiki Extension (1/3): Piwik'
...
echo 'Downloading MediaWiki Extension (3/3): Math'
wget https://extdist.wmflabs.org/dist/extensions/Math-REL1_31-a1263db.tar.gz
tar -xzf Math-REL1_31-a1263db.tar.gz
cd Math
composer install

echo 'Finished downloading (3/3)'

echo 'Downloading Logo'
cd /opt/bitnami/mediawiki/resources
wget toyhouse.cc/wiki/resources/assets/toyhouse.png
mv toyhouse.png wiki.png

echo 'Finish installation by entering the following'
echo 'bash append.sh <PIWIK_URL> <PIWIK_ID_NUMBER> <WP_OAUTH_ID> <WP_OAUTH_SECRET>'
echo 'Replace variables in each <> with its corresponding value'
